...
# file read module
fin = open(r"C:\Main\Study\Sem 5\P346 Computational Phy Lab\Code\Q6 in.txt", "r+")
inp = fin.readlines()
in_mat=[1]*len(inp)
...
#file print module
fout = open(r"C:\Main\Study\Sem 5\P346 Computational Phy Lab\Code\Q6 out.txt", "w+")
fout.writelines("Eigenvalue estimation by Power iteration (Precision: 10^-3)\n\n")
fout.close()

fout = open(r"C:\Main\Study\Sem 5\P346 Computational Phy Lab\Code\Q6 out.txt", "a")
fout.writelines("\nInput matrix:\n")
for i in range(len(in_mat)): 
    for j in range(len(in_mat)):fout.writelines(str(in_mat[i][j])+" ")
    fout.writelines("\n")
fout.writelines("\nThe dominant eigenvalue is "+str(float(f'{eval[1]:.4f}')))
fout.writelines("\nThe corresponding (normalized) eigenvector is:\n")
for i in range(len(evec)):fout.writelines(str(float(f'{evec[i][0]:.4f}'))+"\n")
