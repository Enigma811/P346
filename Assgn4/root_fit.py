...
def bisec(s, eps, de, lb = -1, ub = 1, n = 10000):
    ...
    for k in range(n):
      ...
      if abs(anew) < eps: return lnew, (k+1)
      elif abs(bnew) < eps: return unew, (k+1)
      ...
      #if both ouputs have same sign and f(lnew) = f(unew)
      elif (anew*bnew > 0) & (anew == bnew): unew = (lnew + unew)/2
        ...
        if (anew*c < 0) & (bnew*c > 0):
            unew = ctnew
            if abs(lnew - unew) < de: return ctnew, (k+1)
...
# Regula falsi
def regfal(s, eps, de, lb = -1, ub = 1, n = 10000):
# 's' - input function as string
# 'eps' - tolerance for output of function
# 'de' - tolerance for input of function
# 'lb' - initial guess for lower bound of test interval, default = -1
...
# 'n' - max iteration count
    ...
    for k in range(n):
      ...
      if abs(anew) < eps: return lnew, (k+1)
        ...
        if (anew*c < 0) & (bnew*c > 0):
            unew = ctnew
            if abs(ctnew - ctold) < de: return ctnew, (k+1)
        elif (bnew*c < 0) & (anew*c > 0):
            lnew = ctnew
            if abs(ctnew - ctold) < de: return ctnew, (k+1)
