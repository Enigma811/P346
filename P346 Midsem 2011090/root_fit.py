...
cnt = 0
# Bisection method
def bisec(s, eps, de, lb = -1, ub = 1, n = 10000):
...
# 'n' - max iteration count for testing, default = 10000
    ...
    if (abs(ub - lb) < de) & (abs(b - a) < eps):
      ...
      if c*a < 0: #going with the left half
        if cnt < n: 
          ...
          print("No convergent solution found after",cnt,"iterations")
          return
        return(bisec(s, eps, de, lb, d))
...
# Regula falsi (recursive)
def regfal(s, eps, lb = -1, ub = 1, n = 10000):
# 's' - input function as string
# 'eps' - tolerance for output of function
# 'lb' - initial guess for lower bound of test interval, default = -1
# 'ub' - initial guess for upper bound of test interval, default = 1
# 'n' - max iteration count for testing, default = 10000
  ...
  if a*b < 0:
    ...
    c = eval(s)
    if a*c < 0: 
      return(regfal(s, eps, lb, np))
    elif b*c < 0:
      return(regfal(s, eps, np, ub))
  elif (a*b > 0) & (a > 0):
    if a < b:
      y = ub
      ub = lb
...
def newraph(f, df, eps, de, ini = 0, n = 10000):
