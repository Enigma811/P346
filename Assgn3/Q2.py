...
# file read module
fin = open(r"C:\Main\Study\Sem 5\P346 Computational Phy Lab\Code\Assgn3\Q2 in.txt", "r+")
inp = fin.readlines()
in_mat=[1]*len(inp)
...
if len(in_mat)+1 == len(in_mat[0]):
    ...
    if a == True & b == True: #check for symmetric and pos def matrix
        ...
        for i in range(0, len(A)):
            sum = 0
            for j in range(len(A)-1,-1,-1): sum = sum + (A[i][j]*B[j])
            B[i] = (B[i] - sum)/A[i][i]
        #file print module
    ...
    #file print module
    fout = open(r"C:\Main\Study\Sem 5\P346 Computational Phy Lab\Code\Assgn3\Q2 out.txt", "a")
