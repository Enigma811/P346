...
def is_posdef(val, m=10000):
  ...
  r_arr = rn.myLCG(ct, len(val), 200, -1)
  for i in range(len(val)): 
    test[i][0] = r_arr[i]
  ...
  for k in range(m):
    ...
    if p[0][0] > 0: 
      ...
      ct = time.strftime("%H%M%S", t)
      ct = (int(ct) + k)*int(ct)
      r_arr = rn.myLCG(ct, len(val), -100, -1)
    ...
    if len(x) != len(x[0]):
        ...
        for j in range(0, len(x)):
            for i in range(1, len(x)):
                if i <= j:
                    sum1 = 0
                    for k in range(0,i): sum1 += x[i][k]*x[k][j]
                    x[i][j] = x[i][j] - sum1
                else: 
                    sum2 = 0
                    for k in range(0,j): sum2 += x[i][k]*x[k][j]
...
def cholesky_deco(x):
  # x takes input matrix; pre-augmented
    if len(x)!=len(x[0]):
